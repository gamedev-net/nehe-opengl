...
bool TRay::adjacentPoints(const TRay &ray, TVector &point1, TVector &point2) const {
...

	{
		...
		if (fabs(temp2)<EPSILON) {
			...
			double mu = TVector::dot(_V, _Pt-ray._Pt)/temp;

			point1 = _Pt;

			TVector::add(ray._Pt, TVector::multiply(ray._V, mu, tv), point2);

		} else {

			double a = TVector::dot(_V, TVector::subtract(ray._Pt, _Pt, tv));

			double b = TVector::dot(ray._V, TVector::subtract(_Pt, ray._Pt, tv));

			double mu = (b + temp*a)/temp2;

			double lambda = (a + temp*b)/temp2;

			TVector::add(_Pt, TVector::multiply(_V, lambda, tv), point1);
...
double TRay::dist(const TVector &point) const {
	...
	if (isValid() && point.isValid()) {
		...
		double lambda = TVector::dot(_V, point-_Pt);

		TVector::add(_Pt, TVector::multiply(_V, lambda, tv), point2);
...
ostream &TRay::write(ostream &out) const {
	...
	return isValid() ? out<<"(P="<<_Pt<<",V="<<_V<<")" : out<<"Invalid";
