...
#define TEXTURES_NUM 3       // We Have 3 Textures (CHANGED)
...
GLuint	g_fogMode[]= { GL_EXP, GL_EXP2, GL_LINEAR };    // Storage For Three Types Of Fog (NEW)
GLuint	g_fogfilter = 0;                                // Which Fog Mode To Use (NEW)
GLfloat	g_fogColor[4] = {0.5f,0.5f,0.5f,1.0f};          // Fog Color (NEW)
...
bool setup_textures()
{
	...
	if (!load_rgb_image("crate_256x256.raw", 256, 256, &img)) return false;
...
bool init(void)
{
	...
	glClearColor(0.5f,0.5f,0.5f,1.0f);                 // We'll Clear To The Color Of The Fog (CHANGED)
	...
	glFogi(GL_FOG_MODE, g_fogMode[g_fogfilter]);       // Fog Mode
...
void keyboard(unsigned char key, int x, int y)
{
	switch (key) {
		...
		case 'G': case 'g':
			g_fogfilter += 1;
			if (g_fogfilter > 2) g_fogfilter = 0;
			glFogi(GL_FOG_MODE, g_fogMode[g_fogfilter]); // Fog Mode
		break;
		default:
		break;
