...
(define gl-init
  (lambda ()
    (let ((texture (load-texture "Crate.bmp")))
      (glGenTextures 3 *textures*)

      ;; GL_Nearest filtered texture
...
(define (gl-handlekey key)
  (let ((k (send key get-key-code)))
        (cond ((eq? k 'escape) (exit))
              ...
              ((eq? k #\a)      (set! *xspeed* (- *xspeed* 0.2)))
              ((eq? k #\d)      (set! *xspeed* (+ *xspeed* 0.2)))
              ((eq? k #\q)      (set! *yspeed* (- *yspeed* 0.2)))
              ((eq? k #\e)      (set! *yspeed* (+ *yspeed* 0.2)))
...
(define frame
  (instantiate frame% ()
    (label "NeHe's OpenGL Tutorial #7 - ported by BBurns")
    (width 640)
    (height 480)))
