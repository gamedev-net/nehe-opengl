...
bool initMultitexture(void) {
	char *extensions;	
	extensions=(char *) glGetString(GL_EXTENSIONS);					// Fetch Extension String
...
int LoadGLTextures()									// Load Images And Convert To Textures
{
	...
	ilGenImages(1, &ImgId);								// Create The Image
	ilBindImage(ImgId);									// Bind The Image
	ilutEnable(ILUT_OPENGL_CONV);						// Use GL_RGB8 In glTexImage2D

	// Load The Tile-Bitmap For Base-Texture
	if (ilLoadImage("Data/Base.oil")) {
		...
		ilutGLTexImage(0);								// Send To OpenGL As A Texture
		...
		ilutGLBuildMipmaps();							// Send To OpenGL As Mipmaps
	}
	else status=false;
	
	// Load The Bumpmaps
	if (ilLoadImage("Data/Bump.oil")) {
		...
		glGenTextures(2, bump);										// Create Two Textures

		// Create Nearest Filtered Texture
		glBindTexture(GL_TEXTURE_2D, bump[0]);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
		ilutOglTexImage(0);								// Send To OpenGL As A Texture
		...
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		bump[2] = ilutOglBuildMipmaps();				// Create And Send To OpenGL As Mipmaps
		
		iluNegative();					// Invert The Bumpmap

		glGenTextures(2, invbump);		// Create Two Textures
		...
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
		glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
		invbump[2] = ilutOglBuildMipmaps();				// Create And Send To OpenGL As Mipmaps
	...
	if (ilLoadImage("Data/OpenGL.oil")) {
		glLogo = ilutOglBindTexImage();				// Create And Send To OpenGL As A Texture
	}
	else status=false;
	
	// Load The "Extension Enabled"-Logo
	if (ilLoadImage("Data/multi_on.oil")) {							
		multiLogo = ilutOglBindTexImage();			// Create And Send To OpenGL As A Texture
	}
	else status=false;
