/*                                                                     */
/* This code was created by Jeff Molofee '99 (ported to Irix/GLUT by   */ 
/* Rob Fletcher '00)                                                   */
/*                                                                     */
...
GLuint filter = 0;       /* texture filtering method to use 
                            (nearest, linear, linear + mipmaps)  */
...
/*
   helper for SetupWorld.  reads a file into a string until a
   nonblank, non-comment line is found ("/" at the start indicating
   a comment); assumes lines < 255 characters long.
*/
...
{	
    ...
    if (!ImageLoad("mud.bmp", TextureImage)) {
	exit(1);
    }        
    ...
    glTexParameteri(GL_TEXTURE_2D,
                    ...
                    GL_NEAREST); 		/* scale cheaply when image bigger than texture  */
    glTexParameteri(GL_TEXTURE_2D,
                    GL_TEXTURE_MIN_FILTER,
                    GL_NEAREST); 		/* scale cheaply when image smalled than texture  */
    ...
    glTexParameteri(GL_TEXTURE_2D,
                    GL_TEXTURE_MIN_FILTER,
                    GL_LINEAR_MIPMAP_NEAREST); /* scale mipmap when image smalled than texture  */
    gluBuild2DMipmaps(GL_TEXTURE_2D, 3,
                      TextureImage->sizeX, TextureImage->sizeY, GL_RGB,
