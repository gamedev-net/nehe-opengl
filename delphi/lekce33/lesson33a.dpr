...
var
  ...
  spin: GLFloat;                    // Rotace
  textures: array [0..1] of Texture;// Ukládá textury
...
begin
  Status := false;
  if LoadTGA(textures[0],'Data/Uncompressed.tga') and
     LoadTGA(textures[1],'Data/Compressed.tga') then        // Vše je bez problémù?
    begin
    Status := true;                                         // Vše je bez problémù
    for loop := 0 to 1 do
      begin
      glGenTextures(1,textures[loop].texID);                                    // Generuje texturu
      glBindTexture(GL_TEXTURE_2D,textures[loop].texID);                        // Typické vytváøení textury z bitmapy
      glTexImage2D(GL_TEXTURE_2D,0,textures[loop].bpp div 8,textures[loop].width,textures[loop].height,0,textures[loop].typ,GL_UNSIGNED_BYTE,textures[loop].imageData);    // Vlastní vytváøení textury
      ...
      FreeMem(textures[loop].imageData);
      end;
    end;
...
begin
  ...
  glLoadIdentity();	                                    // Reset matice
  glTranslatef(0.0,0.0,-10.0);                          // Posun do obrazovky
  spin := spin + 0.05;                                  // Rotace
  for loop := 0 to 19 do
    begin
    glPushMatrix;											                  // Uloží matici
		glRotatef(spin+loop*18.0,1.0,0.0,0.0);				      // Rotace na ose X
		glTranslatef(-2.0,2.0,0.0);							            // Posun o 2 jednotky doleva a nahoru
		glBindTexture(GL_TEXTURE_2D,textures[0].texID);			// Výbìr textury
		glBegin(GL_QUADS);										              // Vykreslení ètverce
			glTexCoord2f(0.0,1.0); glVertex3f(-1.0, 1.0,0.0);
			glTexCoord2f(1.0,1.0); glVertex3f( 1.0, 1.0,0.0);
			glTexCoord2f(1.0,0.0); glVertex3f( 1.0,-1.0,0.0);
			glTexCoord2f(0.0,0.0); glVertex3f(-1.0,-1.0,0.0);
		glEnd;												                      // Konec kreslení ètverce
		glPopMatrix;											                  // Obnovení matice
		glPushMatrix;                                       // Uloží matici
		glTranslatef(2.0,0.0,0.0);							            // Posun o 2 jednotky doprava
		glRotatef(spin+loop*36.0,0.0,1.0,0.0);				      // Rotace na ose Y
		glTranslatef(1.0,0.0,0.0);							            // Posun doprava
		glBindTexture(GL_TEXTURE_2D,textures[1].texID);			// Výbìr textury
		glBegin(GL_QUADS);										              // Vykreslení ètverce
			glTexCoord2f(0.0,0.0); glVertex3f(-1.0, 1.0,0.0);
			glTexCoord2f(1.0,0.0); glVertex3f( 1.0, 1.0,0.0);
			glTexCoord2f(1.0,1.0); glVertex3f( 1.0,-1.0,0.0);
