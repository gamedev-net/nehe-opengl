...
uses
  ...
  OpenGL,
  model in 'model.pas',
  MilkshapeModel in 'MilkshapeModel.pas';
...
var
  ...
  yrot: GLFloat = 0.0;              // Rotace v ose y
  pModel: TModel = nil;             // Ukazatel na model
...
begin
  pModel.reloadTexture;                             // Nahrání textur
...
begin
  ...
  glLoadIdentity();	                                    // Reset matice
  gluLookAt(75,75,75,0,0,0,0,1,0);                      // Pøesun kamery
  glRotatef(yrot,0.0,1.0,0.0);                          // Rotace na ose y
  pModel.Draw;                                          // Rendering modelu
  yrot := yrot + 1.0;                                   // Otáèení scény
...
begin
  ...
  pModel := TMilkshapeModel.Create;                   // Alokace pamìti pro model
  if not pModel.loadModelData('Data/Model.ms3d') then // Pokusí se nahrát model
    begin
    MessageBox(0,'Couldn''t load the model data/model.ms3d','Error',MB_OK or MB_ICONERROR);
    Result := 0;                                      // Model se nepodaøilo nahrát - program se ukonèí
    exit;
    end;
