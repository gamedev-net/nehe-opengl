...
begin
  ...
  PixelFormat := ChoosePixelFormat(window.hDc,@pfd);       // Zkusí najít Pixel Format
  ...
  if QueryPerformanceFrequency(timer) then			              // Mùžeme použít Peformance Counter?
	  begin
		window.hrTimer := true;											              // Èítaè s vysokým rozlišením je dostupný
		window.lastTickCount := DWORD(QueryPerformanceCounter(timer));   // Poèáteèní hodnota èítaèe
		QueryPerformanceFrequency(timer);                         // Frekvence èítaèe
		window.timerResolution := 1.0 / timer;                    // Nastaví rozlišení èítaèe
	  end
    else
    begin
    window.lastTickCount := GetTickCount;
    window.timerResolution := 1.0 / 1000.0;
    end;
