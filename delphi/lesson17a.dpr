...
var
  ...
  base: GLuint;                     // Èíslo základního display listu znakù
  loop: GLuint;                     // Pomocná pro cykly
  texture: array[0..1] of GLuint;   // Ukládá textury
...
begin
  ...
  TextureImage[1] := LoadBMP('Data/Bumps.bmp');             // Nahraje bitmapu
...
var cx, cy: GLfloat;                                      // Koordináty x, y
begin
  ...
  for loop:=0 to 255 do                                   // Vytváøí 256 display listù
    ...
    glNewList(base + loop,GL_COMPILE);                    // Vytvoøení display listu
      glBegin(GL_QUADS);                                  // Pro každý znak jeden obdélník
        glTexCoord2f(cx,1-cy-0.0625);glVertex2i(0,0);           // Levý dolní
        glTexCoord2f(cx+0.0625,1-cy-0.0625);glVertex2i(16,0);   // Pravý dolní
        glTexCoord2f(cx+0.0625,1-cy);glVertex2i(16,16);         // Pravý horní
...
procedure glPrint(x,y: GLint;text: string;sada: integer);
begin
  ...
  glListBase(base-32+(128*sada));                         // Zvolí znakovou sadu (0 nebo 1)
...
begin
  ...
  glLoadIdentity();	                                    // Reset matice
  glBindTexture(GL_TEXTURE_2D,texture[1]);              // Výbìr textury
  glTranslatef(0.0,0.0,-5.0);                           // Pøesun o pìt do obrazovky
  glRotatef(45.0,0.0,0.0,1.0);                          // Rotace o 45° po smìru hodinových ruèièek na ose z
  glRotatef(cnt1*30.0,1.0,1.0,0.0);                     // Rotace na osách x a y
  ...
  glRotatef(90.0,1.0,1.0,0.0);                          // Rotace na osách X a Y o 90°
  glBegin(GL_QUADS);                                    // Kreslení obdélníku
    glTexCoord2d(0.0,0.0);
    glVertex2f(-1.0, 1.0);
    glTexCoord2d(1.0,0.0);
  ...
  glLoadIdentity();                                     // Reset matice
  // Zmìna barvy založená na pozici textu
  glColor3f(1.0*(cos(cnt1)),1.0*(sin(cnt2)),1.0-0.5*(cos(cnt1+cnt2)));
  glPrint(trunc((280+250*cos(cnt1))),trunc(235+200*sin(cnt2)),'NeHe',0);		// Vypíše text
  glColor3f(1.0*(sin(cnt2)),1.0-0.5*(cos(cnt1+cnt2)),1.0*(cos(cnt1)));
  glPrint(trunc((280+230*cos(cnt2))),trunc(235+200*sin(cnt1)),'OpenGL',1);	// Vypíše text
  glColor3f(0.0,0.0,1.0);                                                   // Modrá barva
  glPrint(trunc(240+200*cos((cnt2+cnt1)/5)),2,'Giuseppe D''Agata',0);       // Vypíše text
  glColor3f(1.0,1.0,1.0);                                                   // Bílá barva
  glPrint(trunc(242+200*cos((cnt2+cnt1)/5)),2,'Giuseppe D''Agata',0);       // Vypíše text
  cnt1 := cnt1 + 0.01;										
  cnt2 := cnt2 + 0.0081;
