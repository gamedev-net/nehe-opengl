...
begin
  ...
  TextureImage := LoadBMP('Data/Glass.bmp');                               // Nahraje bitmapu
...
begin
  ...
  glColor4f(1.0,1.0,1.0,0.5);                       // Pln˝ jas, 50% alfa
  glBlendFunc(GL_SRC_ALPHA,GL_ONE);                 // Funkce blendingu pro pr˘svitnost zaloûen· na hodnotÏ alfa
    ...
    begin
      if (PeekMessage(msg, 0, 0, 0, PM_REMOVE)) then  // P¯iöla zpr·va?
        ...
        begin
          ...
          if (keys[ord('B')] and not bp) then         // Kl·vesa B - zapne blending
            begin
            bp := true;
            blend := not blend;
            if not blend then
              ...
              glEnable(GL_DEPTH_TEST);                // Zapne hloubkovÈ testov·nÌ
              end
              else
              begin
          ...
          if not keys[ord('B')] then bp := false;     // UvolnÏnÌ B
        end;
