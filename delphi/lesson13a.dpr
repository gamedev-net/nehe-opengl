...
begin
  ...
  wglUseFontBitmaps(h_dc,32,96,base);                     // Vytvoøí 96 znakù, poèínaje 32 v Ascii
end;

procedure KillFont;                                       // Smaže font
begin
  glDeleteLists(base,96);                                 // Smaže všech 96 znakù (display listù)
end;
...
begin
  ...
  glLoadIdentity();	                                    // Reset matice
  glTranslatef(0.0,0.0,-1.0);                           // Pøesun o 1 do obrazovky
  glColor3f(1.0*cos(cnt1),1.0*sin(cnt2),1.0-0.5*cos(cnt1+cnt2));  // Pulzování barev závislé na pozici
  glRasterPos2f(-0.45+0.05*cos(cnt1), 0.32*(sin(cnt2)));          // Pozice textu
  glPrint(Format('Active OpenGL Text With NeHe - %7.2f',[cnt1])); // Výpis textu
  cnt1 := cnt1 + 0.051;
  cnt2 := cnt2 + 0.005;
