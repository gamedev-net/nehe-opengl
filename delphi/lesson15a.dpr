...
begin
  ...
  SelectObject(h_dc,font);                                // Vıbìr fontu do DC
  wglUseFontOutlines(h_dc,                                // Vybere DC
                      ...
                      0.0,                                // Hranatost
                      0.2,                                // Hloubka v ose z
                      WGL_FONT_POLYGONS,                  // Polygony ne drátìnı model
                      @gmf);                              // Adresa bufferu pro uloení informací
end;

procedure KillFont;                                       // Smae font
begin
  glDeleteLists(base,256);                                // Smae všech 256 znakù (display listù)
end;
...
begin
  ...
  TextureImage[0] := LoadBMP('Data/Lights.bmp');            // Nahraje bitmapu
  if Assigned(TextureImage[0]) then                         // Vše je bez problémù?
    ...
    glBindTexture(GL_TEXTURE_2D,texture[0]);                // Vytvoøí lineárnì mipmapovanou texturu
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR_MIPMAP_NEAREST);
    gluBuild2DMipmaps(GL_TEXTURE_2D,3,TextureImage[0].sizeX,TextureImage[0].sizeY,GL_RGB,GL_UNSIGNED_BYTE,TextureImage[0].data);
    glTexGeni(GL_S, GL_TEXTURE_GEN_MODE, GL_OBJECT_LINEAR); // Automatické generování koordinátù textury
...
begin
  ...
  delka := 0;
  for loop:=1 to length(text)-1 do delka := delka + gmf[loop].gmfCellIncX;  // Inkrementace o šíøku znaku
  glTranslatef(-delka/2,0.0,0.0);                         // Zarovnání na støed
  glPushAttrib(GL_LIST_BIT);                              // Uloí souèasnı stav display listù
...
begin
  ...
  glEnable(GL_LIGHT0);                              // Zapne implicitní svìtlo
...
begin
  ...
  glRotatef(rot,1.0,0.0,0.0);                           // Rotace
  glRotatef(rot*1.2,0.0,1.0,0.0);
  glRotatef(rot*1.4,0.0,0.0,1.0);
  glTranslatef(-0.35,-0.35,0.1);                        // Vycentrování
  glPrint('N');                                         // Vykreslí lebku a zkøíené kosti
  rot := rot + 0.1;                                     // Inkrementace rotace a pohybu
