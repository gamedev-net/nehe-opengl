...
begin
  ...
  SelectObject(h_dc,font);                                // Výbìr fontu do DC
...
begin
  ...
  glLoadIdentity();	                                    // Reset matice
  glTranslatef(0.0,0.0,-10.0);                          // Pøesun do obrazovky
  ...
  glColor3f(1.0*(cos(rot/20.0)),1.0*(sin(rot/25.0)),1.0-0.5*(cos(rot/17.0))); // Pulzování barev závislé na pozici a rotaci
  glPrint(Format('NeHe - %3.2f',[rot/50]));             // Výpis textu
  rot := rot + 0.5;                                     // Inkrementace èítaèe
