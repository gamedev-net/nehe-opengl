...
var
  ...
  lp: bool;                         // Stisknuto L?
  fp: bool;                         // Stisknuto F?
  sp: bool;                         // Stisknutý mezerník?
  part1: integer;                   // Zaèátek disku
  part2: integer;                   // Konec disku
  p1: integer = 0;                  // Pøírùstek 1
  p2: integer = 1;                  // Pøírùstek 2
  ...
  filter: gluint;                   // Specifikuje používaný texturový filtr
  texture: array [0..2] of GLuint;  // Ukládá tøi textury
  objekt: gluint = 0;               // Urèuje aktuálnì vykreslovaný objekt
...
begin
  ...
  TextureImage := LoadBMP('Data/Wall.bmp');                           // Nahraje bitmapu
...
begin
  ...
  case objekt of                                        // Vybere, co se bude kreslit
    ...
    2: gluDisk(quadratic,0.5,1.5,32,32);                // Disk ve tvaru CD
    3: gluSphere(quadratic,1.3,32,32);                  // Koule
    4: begin
       glTranslatef(0.0,0.0,-1.5);                      // Vycentrování kužele
       gluCylinder(quadratic,1.0,0.0,3.0,32,32);        // Kužel
       end;
    5: begin
       part1 := part1 + p1;                             // Inkrementace poèáteèního úhlu
       part2 := part2 + p2;                             // Inkrementace pøírùstkového úhlu
       if part1 > 359 then                              // 360°
        begin
        part1 := 0;                                     // Vynulování poèáteèního úhlu
        p1 := 0;                                        // Zastaví zvìtšování poèáteèního úhlu
        part2 := 0;                                     // Vynulování pøírùstkového úhlu
        p2 := 1;                                        // Zaène zvìtšovat pøírùstkový úhel
        end;
       if part2 > 359 then                              // 360°
        begin
        p1 := 1;                                        // Zaène zvìtšovat poèáteèní úhel
        p2 := 0;                                        // Pøestane zvìtšovat pøírùstkový úhel
        end;
       gluPartialDisk(quadratic,0.5,1.5,32,32,part1,part2-part1); // Neúplný disk
       end;
    end;
    ...
    begin
      if (PeekMessage(msg, 0, 0, 0, PM_REMOVE)) then  // Pøišla zpráva?
        ...
        begin
          ...
          if keys[VK_PRIOR] then z := z - 0.02;       // Klávesa Page Up - zvýší zanoøení do obrazovky
          if keys[VK_NEXT] then z := z + 0.02;        // Klávesa Page Down - sníží zanoøení do obrazovky
          ...
          if (keys[ord(' ')] and not sp) then         // Stisknutý mezerník?
            begin
            sp := true;
            objekt := objekt + 1;                     // Cyklování objekty
            if objekt > 5 then objekt := 0;           // Ošetøení pøeteèení
            end;
