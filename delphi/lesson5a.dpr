...
var
  ...
  rtri: GLfloat = 0;                // Úhel pro trojúhelník
  rquad: GLfloat = 0;               // Úhel pro ètverec
...
begin
  ...
  glRotatef(rtri,0.0,1.0,0.0);                          // Otoèí trojúhelník okolo osy y
  glBegin(GL_triangles);                                // Zaèátek kreslení pyramidy
    glColor3f(1.0,0.0,0.0);                             // Èervená
    glVertex3f( 0.0, 1.0, 0.0);                         // Horní bod (èelní stìna)
    glColor3f(0.0,1.0,0.0);                             // Zelená
    glVertex3f(-1.0,-1.0, 1.0);                         // Levý spodní bod (èelní stìna)
    glColor3f(0.0,0.0,1.0);                             // Modrá
    glVertex3f( 1.0,-1.0, 1.0);                         // Pravý spodní bod (èelní stìna)

    glColor3f(1.0,0.0,0.0);                             // Èervená
    glVertex3f( 0.0, 1.0, 0.0);                         // Horní bod (pravá stìna)
    glColor3f(0.0,0.0,1.0);                             // Modrá
    glVertex3f( 1.0,-1.0, 1.0);                         // Levý bod (pravá stìna)
    glColor3f(0.0,1.0,0.0);                             // Zelená
    glVertex3f( 1.0,-1.0, -1.0);                        // Pravý bod (pravá stìna)

    glColor3f(1.0,0.0,0.0);                             // Èervená
    glVertex3f( 0.0, 1.0, 0.0);                         // Horní bod (zadní stìna)
    glColor3f(0.0,1.0,0.0);                             // Zelená
    glVertex3f( 1.0,-1.0, -1.0);                        // Levý bod (zadní stìna)
    glColor3f(0.0,0.0,1.0);                             // Modrá
    glVertex3f(-1.0,-1.0, -1.0);                        // Pravý bod (zadní stìna)

    glColor3f(1.0,0.0,0.0);                             // Èervená
    glVertex3f( 0.0, 1.0, 0.0);                         // Horní bod (levá stìna)
    glColor3f(0.0,0.0,1.0);                             // Modrá
    glVertex3f(-1.0,-1.0,-1.0);                         // Levý bod (levá stìna)
    glColor3f(0.0,1.0,0.0);                             // Zelená
    glVertex3f(-1.0,-1.0, 1.0);                         // Pravý bod (levá stìna)
  GlEnd();                                              // Konec kreslení pyramidy
  glLoadIdentity();                                     // Reset matice
  glTranslatef(1.5,0.0,-7.0);                           // Posun poèátku
  glRotatef(rquad,1.0,1.0,1.0);                         // Pootoèení ètverce okolo osy x
  glBegin(GL_QUADS);                                    // Zaèátek kreslení krychle
    glColor3f(0.0,1.0,0.0);                             // Zelená
    glVertex3f( 1.0, 1.0,-1.0);                         // Pravý horní (horní stìna)
    glVertex3f(-1.0, 1.0,-1.0);                         // Levý horní (horní stìna)
    glVertex3f(-1.0, 1.0, 1.0);                         // Levý dolní (horní stìna)
    glVertex3f( 1.0, 1.0, 1.0);                         // Pravý dolní (horní stìna)

    glColor3f(1.0,0.5,0.0);                             // Oranžová
    glVertex3f( 1.0,-1.0, 1.0);                         // Pravý horní bod (spodní stìna)
    glVertex3f(-1.0,-1.0, 1.0);                         // Levý horní (spodní stìna)
    glVertex3f(-1.0,-1.0,-1.0);                         // Levý dolní (spodní stìna)
    glVertex3f( 1.0,-1.0,-1.0);                         // Pravý dolní (spodní stìna)

    glColor3f(1.0,0.0,0.0);                             // Èervená
    glVertex3f( 1.0, 1.0, 1.0);                         // Pravý horní (èelní stìna)
    glVertex3f(-1.0, 1.0, 1.0);                         // Levý horní (èelní stìna)
    glVertex3f(-1.0,-1.0, 1.0);                         // Levý dolní (èelní stìna)
    glVertex3f( 1.0,-1.0, 1.0);                         // Pravý dolní (èelní stìna)

    glColor3f(1.0,1.0,0.0);                             // Žlutá
    glVertex3f( 1.0,1.0,-1.0);                          // Pravý horní (zadní stìna)
    glVertex3f(-1.0,1.0,-1.0);                          // Levý horní (zadní stìna)
    glVertex3f(-1.0,-1.0,-1.0);                         // Levý dolní (zadní stìna)
    glVertex3f( 1.0,-1.0,-1.0);                         // Pravý dolní (zadní stìna)

    glColor3f(0.0,0.0,1.0);                             // Modrá
    glVertex3f(-1.0, 1.0, 1.0);                         // Pravý horní (levá stìna)
    glVertex3f(-1.0, 1.0,-1.0);                         // Levý horní (levá stìna)
    glVertex3f(-1.0,-1.0,-1.0);                         // Levý dolní (levá stìna)
    glVertex3f(-1.0,-1.0, 1.0);                         // Pravý dolní (levá stìna)

    glColor3f(1.0,0.0,1.0);                             // Fialová
    glVertex3f( 1.0, 1.0,-1.0);                         // Pravý horní (pravá stìna)
    glVertex3f( 1.0, 1.0, 1.0);                         // Levý horní (pravá stìna)
    glVertex3f( 1.0,-1.0, 1.0);                         // Levý dolní (pravá stìna)
    glVertex3f( 1.0,-1.0,-1.0);                         // Pravý dolní (pravá stìna)
  glEnd();                                              // Konec kreslení krychle
  rtri := rtri + 0.2;                                   // Inkrementace úhlu pootoèení trojúhelníku
  rquad := rquad - 0.15;                                // Inkrementace úhlu pootoèení ètverce
