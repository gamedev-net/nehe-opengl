...
int LoadGLTextures( )
{
    ...
    if ( ( TextureImage[0] = SDL_LoadBMP( "data/nehe.bmp" ) )!=null )
        {
...
bool InitGL()															// Any OpenGL Initialization Code Goes Here
{
   ...
   if ( !LoadGLTextures( ) )									// Load in the texture
		return FALSE;												// If there is a problem loading the texture quit
...
int main(char[][] argv)
{
   ...
   CreateWindowGL(w, h, bpp, video_flags);					//Create our OpenGL Window
	...
	SDL_WM_SetCaption( "NeHe Lesson 06", "Lesson 06" );			//Set the window manager title bar
