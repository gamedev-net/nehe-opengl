...
int LoadGLTextures(){												// Load Bitmaps And Convert To Textures
	...
	if (Image=auxDIBImageLoad("Data/OpenGL_ALPHA.bmp")) {							
		...
		for (int a_b=0; a_b<Image->sizeX*Image->sizeY; a_b++)
			alpha[4*a_b+3]=Image->data[a_b*3];					// Pick Only Red Value As Alpha!
		if (!(Image=auxDIBImageLoad("Data/OpenGL.bmp"))) status=false;
		for (int a_m=0; a_m<Image->sizeX*Image->sizeY; a_m++) {		
			alpha[4*a_m]=Image->data[a_m*3];					// R
			alpha[4*a_m+1]=Image->data[a_m*3+1];				// G
			alpha[4*a_m+2]=Image->data[a_m*3+2];				// B
		}
	...
	if (Image=auxDIBImageLoad("Data/multi_on_alpha.bmp")) {							
		...
		for (int a_h=0; a_h<-Image->sizeX*Image->sizeY; a_h++) {		
			alpha[4*a_h]=Image->data[a_h*3];					// R
			alpha[4*a_h+1]=Image->data[a_h*3+1];				// G
			alpha[4*a_h+2]=Image->data[a_h*3+2];				// B
		}
