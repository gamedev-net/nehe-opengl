...
int LoadGLTextures()									// Load Bitmaps And Convert To Textures
{
	...
	for (int loop_m = 0; loop_m < 2; loop_m++)
		{
	        if (TextureImage[loop_m])							// If Texture Exists
		    {
			        if (TextureImage[loop_m]->data)			// If Texture Image Exists
				    {
					        free(TextureImage[loop_m]->data);	// Free The Texture Image Memory
					}
					free(TextureImage[loop_m]);				// Free The Image Structure
			}
		}
	}
