...
int LoadGLTextures()                                    // Load Bitmaps And Convert To Textures
{
	...
	{   
		...
		for (int loop_b=0; loop_b<3; loop_b++)					// Loop Through 5 Textures
		{
			if (TextureImage[loop_b])						// If Texture Exists
			{
				if (TextureImage[loop_b]->data)			// If Texture Image Exists
				{
					free(TextureImage[loop_b]->data);		// Free The Texture Image Memory
				}
				free(TextureImage[loop_b]);				// Free The Image Structure 
			}
		}
	}
