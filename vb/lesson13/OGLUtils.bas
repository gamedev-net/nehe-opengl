...
Public Sub BuildFont(frm As Form)                    ' Build Our Bitmap Font
    ...
    hfont = CreateFont(-24, 0, 0, 0, FW_BOLD, False, False, False, _
            ANSI_CHARSET, OUT_TT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_DONTCARE Or DEFAULT_PITCH, "Courier New")
            

    SelectObject frm.hDC, hfont                ' Selects The Font We Created ( NEW )

    wglUseFontBitmaps frm.hDC, 32, 96, base           ' Builds 96 Characters Starting At Character 32 ( NEW )
...
Public Sub glPrint(ByVal s As String)                ' Custom GL "Print" Routine
    ...
    If Len(s) > 0 Then              ' only if the pass a string
        ...
        For i = 1 To Len(s)         ' for each character
            b(i) = Asc(Mid$(s, i, 1))   ' convert from unicode to ascii
        Next
