...
LIBRARIES = -lX11 -lXi -lXmu -lGL -lGLU -lm -lstdc++ -ldl
...
#
all: $(basename $(wildcard lesson[1-9].c lesson1[0-9].c lesson2[0-5].c))
...
# to send to nehe.
#
dist: $(foreach file,$(basename $(wildcard lesson[1-9].c lesson1[0-9].c lesson2[0-5].c)),$(file).tar.gz)
...
clean:
	rm $(wildcard lesson[1-9] lesson1[0-3] lesson18)

# to clean up the distributions:
# delete all of the lessonX.tar.gz files.
distclean:
	rm $(wildcard lesson[1-9].tar.gz lesson1[0-9].tar.gz lesson2[0-5].tar.gz)
